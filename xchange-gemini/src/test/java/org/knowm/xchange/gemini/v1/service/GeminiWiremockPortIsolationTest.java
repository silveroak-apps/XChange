...
/**
 * Regression guard for the WireMock port collision that broke the Gemini suite on the shared
 * self-hosted CI runner.
 *
 * <p>The runner already listens on 8080 (the deployed price synchroniser serves its /healtz endpoint
 * there), so every WireMock-backed Gemini test died at startup with a {@code BindException} when the
 ...
 */
public class GeminiWiremockPortIsolationTest extends BaseWiremockTest {
  ...
  public void baseRuleLeavesPortSelectionToTheOs() {
    assertThat(wireMockRule.getOptions().portNumber())
        .as(
            "Gemini's WireMock harness must request an ephemeral port, not the fixed default %s",
